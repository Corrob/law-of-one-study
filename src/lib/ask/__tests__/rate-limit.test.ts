// Mock the Redis client so tests exercise the in-memory fallback without
// pulling in @upstash/redis (ESM that Jest would otherwise fail to transform).
jest.mock("../redis", () => ({ redis: null, isRedisConfigured: false }));

import { checkRateLimit, getClientIp } from "../rate-limit";

describe("rate-limit", () => {
  describe("checkRateLimit", () => {
    // No Redis env in tests, so this exercises the in-memory fallback.
    it("allows requests up to the limit, then blocks", async () => {
      ...
      await checkRateLimit(id, config); // 2nd
      const r3 = await checkRateLimit(id, config); // 3rd
      expect(r3.success).toBe(true);
      expect(r3.remaining).toBe(0);

      const r4 = await checkRateLimit(id, config); // 4th — over
    ...
    it("resets after the window elapses", async () => {
      ...
      expect((await checkRateLimit(id, past)).success).toBe(true);
