 ...
 * (e.g. `{{CITE:6.14}}`). We resolve those to deep links on lawofone.info —
 * the one site L/L Research authorized to host the full session text — so the
 * reader goes to the authoritative source. We never render the Ra Material
 * verbatim ourselves.
 *
 ...
 * the concept graph and validated against lawofone.info).
 */
...
/**
 * Build the lawofone.info deep link for a `session.question` reference.
 * Format: https://www.lawofone.info/s/{session}#{question}
 */
export function lawOfOneUrl(reference: string): string {
  const [session, question] = reference.split(".");
  return `https://www.lawofone.info/s/${session}#${question}`;
}

/**
 * Replace `{{CITE:ref}}` markers in a response with Markdown links to
 * lawofone.info. Unknown references are dropped (not linked). Any incomplete
 * trailing marker is stripped so it isn't shown mid-stream.
 *
 * Safe to call on partial text on every streaming tick — only complete markers
 * with known references are transformed.
 */
export function renderCitationsToMarkdown(text: string): string {
  const withLinks = text.replace(CITE_MARKER, (_match, reference: string) => {
    if (!KNOWN_REFERENCES.has(reference)) return "";
    return `[${reference}](${lawOfOneUrl(reference)})`;
  });
