 ...
 * lawofone.info so the reader can read the original at the authorized source.
 */
...
function languageInstruction(locale: AvailableLanguage): string {
  ...
  return `LANGUAGE: Respond in ${name}. The source links (lawofone.info) are in English; you may briefly note that where natural. Citation markers stay as-is.`;
}
